...
void local_optimization_with_2flip()
{
    ...
    int *df,neighbor_num,neighbor_idx,neighbor_weight;
    int df2 = 0;
    df = (int *)malloc(sizeof(int)*vertex_num);
    idx = (int *)malloc(sizeof(int)*vertex_num);
    for(i=0;i<vertex_num;i++)
        idx[i] = i;
    ...
    while(improved)
    {
        ...
        if(improved==0)
        {
            ...
            {
                int df2 =0;
                df2 = df[in_edge[i].v1_num - 1] + df[in_edge[i].v2_num - 1];
                if(selection_1.genetic[in_edge[i].v1_num -1] == selection_1.genetic[in_edge[i].v2_num - 1])
                    df2 -= 2*in_edge[i].value;
                else 
                    df2 += 2*in_edge[i].value;
                if(df2 > 0)
                {
                    selection_1.genetic[in_edge[i].v1_num -1] = 1 - selection_1.genetic[in_edge[i].v1_num -1];
                    selection_1.genetic[in_edge[i].v2_num -1] = 1 - selection_1.genetic[in_edge[i].v2_num -1];
                    improved = 1;
                    break;
                }
